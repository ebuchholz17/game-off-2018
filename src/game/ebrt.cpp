...
void initBackBuffer () {
    ...
    bitmapInfo.bmiHeader.biHeight = -imageHeight; // Make the bitmap start from the top left corner
    bitmapInfo.bmiHeader.biPlanes = 1;
    bitmapInfo.bmiHeader.biBitCount = 32;
    bitmapInfo.bmiHeader.biCompression = BI_RGB;

    backBuffer = malloc(4 * imageWidth * imageHeight);
    if (backBuffer == 0) {
    ...
    for (int i = 0; i < imageHeight; ++i) {
        ...
        for (int j = 0; j < imageWidth; ++j) {
            //pixels[i * imageWidth + j] = value << 8;
            //++value;
            pixels[i * imageWidth + j] = value;
        }
    }
}

// TODO(ebuchholz): read in texture coordinates
mesh loadMeshFromOBJ (char *fileName) {
    ...
    if (fopen_s(&file, fileName, "r") != 0) {
        ...
        // TODO(ebuchholz): probably should validate all these pointers
        // TODO(ebuchholz): allocate all of these guys right next to each other
        // TODO(ebuchholz): memory arena
        vector3 *tempPositions = (vector3 *)malloc(numPositions * sizeof(vector3));
        unsigned int tempPositionIndex = 0;
        vector3 *tempNormals = (vector3 *)malloc(numNormals * sizeof(vector3));
        unsigned int tempNormalIndex = 0;

        vector3 *meshPositions = (vector3 *)malloc(3 * numTriangles * sizeof(vector3));
        unsigned int numMeshPositions = 0;
        vector3 *meshNormals = (vector3 *)malloc(3 * numTriangles * sizeof(vector3));
        unsigned int numMeshNormals = 0;
        unsigned int *meshIndices = (unsigned int *)malloc(3 * numTriangles * 
                                                           sizeof(unsigned int));
        ...
        while (!feof(file)) {
            ...
            if (strcmp(buffer, "v") == 0) {
                ...
                tempPositions[tempPositionIndex++] = position;
            }
            else if (strcmp(buffer, "vt") == 0) {
                // ignore for now
            }
                ...
                normal.z = -normal.z;
                tempNormals[tempNormalIndex++] = normal;
            }
            else if (strcmp(buffer, "f") == 0) {
                for (int i = 0; i < 3; ++i) {
                    unsigned int positionIndex, textureCoordIndex, normalIndex;
                    fscanf_s(file, "%d/%d/%d", &positionIndex, &textureCoordIndex, &normalIndex);
                    meshPositions[numMeshPositions++] = tempPositions[positionIndex-1];
                    meshNormals[numMeshNormals++] = tempNormals[normalIndex-1];
                    meshIndices[numMeshIndices++] = numMeshIndices;
                }
                // QQQ generate our own normls
                //vector3 newNormal = crossProduct(meshPositions[numMeshPositions-1] - meshPositions[numMeshPositions-2],
                //                                 meshPositions[numMeshPositions-1] - meshPositions[numMeshPositions-3]);
                //newNormal = normalize(newNormal);
                //meshNormals[numMeshNormals++] = newNormal;
                //meshNormals[numMeshNormals++] = newNormal;
                //meshNormals[numMeshNormals++] = newNormal;
                    
                fgets(buffer, 256, file); // skip rest of line
            }
            else {
                fgets(buffer, 256, file);
            }
        }
        result.positions = meshPositions;
        result.normals = meshNormals;
        result.indices = meshIndices;
        result.numTriangles = numTriangles;

        // change winding of triangles
        for (unsigned int i = 0; i < numTriangles; ++i) {
            unsigned int temp = result.indices[i*3+1];
            result.indices[i*3+1] = result.indices[i*3+2];
            result.indices[i*3+2] = temp;
        }
        free(tempPositions);
        free(tempNormals);

        printf("mesh: %s, num triangles: %d\n", fileName, numTriangles);
    }
...
void initScene () {
    ...
    triangles[1] = Triangle(Vector3(3.0f, 2.0f, 4.0f),
                            ...
                            Vector3(-3.0f, 0.0f, 4.0f));
    //meshes[0] = loadMeshFromOBJ("tree.obj");
    //meshes[0] = loadMeshFromOBJ("castle.obj");
    //meshes[0] = loadMeshFromOBJ("tree (2).obj");
    //meshes[0] = loadMeshFromOBJ("robotsmall.obj");
    meshes[0] = loadMeshFromOBJ("simplebunny.obj");
    //meshes[0] = loadMeshFromOBJ("pumpkin.obj");
    //meshes[0] = loadMeshFromOBJ("lobster.obj");
    //meshes[0] = loadMeshFromOBJ("dragon.obj");
    //meshes[0] = loadMeshFromOBJ("tigerfighter.obj");
    ...
    objects[2].shapeType = MESH;
    objects[2].shapeIndex = 0;
    objects[2].materialIndex = 4;
    //objects[2].transform = identityMatrix;
    objects[2].transform = translationMatrix(0.0f, 0.0f, 0.0f) *
        //rotationMatrixFromAxisAngle(Vector3(0.0f, 1.0f, 0.0f), PI * -0.5f) *
        scaleMatrix(1.0f);

    //objects[2].transform = translationMatrix(0.0f, 1.25f, 0.0f) * scaleMatrix(0.125f);
    //objects[2].transform = translationMatrix(0.0f, 0.75f, 0.0f) * scaleMatrix(0.05f);
    //objects[2].transform = identityMatrix4x4();
    objects[2].inverseTransform = inverse(objects[2].transform);

    theOneKDTreeInTheScene = buildKDTree(objects[2]);

    objects[3].shapeType = PLANE;
    objects[3].shapeIndex = 0;
    objects[3].materialIndex = 3;
    objects[3].transform = identityMatrix;
    objects[3].inverseTransform = identityMatrix;

    objects[4].shapeType = TRIANGLE;
    objects[4].shapeIndex = 0;
    objects[4].materialIndex = 2;
    objects[4].transform = identityMatrix;
    objects[4].inverseTransform = identityMatrix;

    objects[5].shapeType = TRIANGLE;
    objects[5].shapeIndex = 1;
    objects[5].materialIndex = 2;
    objects[5].transform = identityMatrix;
    objects[5].inverseTransform = identityMatrix;
}
void drawSceneToWindow (HWND window, HDC deviceContext) {
    ...
    StretchDIBits(deviceContext, 0 , 0, windowWidth, windowHeight, 0, 0, imageWidth, imageHeight, backBuffer, 
...
void drawLine (int startX, int startY, int endX, int endY) {
    ...
    color.g = 1.0;
    color.b = 1.0;
    unsigned int finalColor = ((int)(255.0f * color.r)) << 16 |
...
void draw3DLine (float startX, float startY, float startZ, float endX, float endY, float endZ, 
...
inline void drawAABB (aabb box, matrix4x4 viewMat, matrix4x4 projMat) {
    draw3DLine(box.min.x, box.min.y, box.min.z, box.max.x, box.min.y, box.min.z, viewMat, projMat);
    draw3DLine(box.max.x, box.min.y, box.min.z, box.max.x, box.min.y, box.max.z, viewMat, projMat);
    draw3DLine(box.max.x, box.min.y, box.max.z, box.min.x, box.min.y, box.max.z, viewMat, projMat);
    draw3DLine(box.min.x, box.min.y, box.max.z, box.min.x, box.min.y, box.min.z, viewMat, projMat);

    draw3DLine(box.min.x, box.max.y, box.min.z, box.max.x, box.max.y, box.min.z, viewMat, projMat);
    draw3DLine(box.max.x, box.max.y, box.min.z, box.max.x, box.max.y, box.max.z, viewMat, projMat);
    draw3DLine(box.max.x, box.max.y, box.max.z, box.min.x, box.max.y, box.max.z, viewMat, projMat);
    draw3DLine(box.min.x, box.max.y, box.max.z, box.min.x, box.max.y, box.min.z, viewMat, projMat);

    draw3DLine(box.min.x, box.min.y, box.min.z, box.min.x, box.max.y, box.min.z, viewMat, projMat);
    draw3DLine(box.max.x, box.min.y, box.min.z, box.max.x, box.max.y, box.min.z, viewMat, projMat);
    draw3DLine(box.max.x, box.min.y, box.max.z, box.max.x, box.max.y, box.max.z, viewMat, projMat);
    draw3DLine(box.min.x, box.min.y, box.max.z, box.min.x, box.max.y, box.max.z, viewMat, projMat);
}

inline void drawKDTreeNode (kdtree *tree, unsigned int nodeIndex, aabb bounds, matrix4x4 viewMat, matrix4x4 projMat) {
    drawAABB(bounds, viewMat, projMat);
    kdtree_node currentNode = tree->nodes[nodeIndex];
    kdtree_split_direction splitDirection = currentNode.splitDirection;
    bool intersected = false;
    if (splitDirection != KDTREE_SPLIT_LEAF) {
        if (currentNode.firstChildIndex != KDTREE_NODE_EMPTY) {
            ...
            leftBounds.max.c[splitDirection] = currentNode.splitPos;
            drawKDTreeNode(tree, currentNode.firstChildIndex, leftBounds, viewMat, projMat);
        }
        if (currentNode.secondChildIndex != KDTREE_NODE_EMPTY) {
            aabb rightBounds = bounds;
            rightBounds.min.c[splitDirection] = currentNode.splitPos;
            drawKDTreeNode(tree, currentNode.secondChildIndex, rightBounds, viewMat, projMat);
        }
    }
}

inline void drawKDTree (kdtree *tree, matrix4x4 viewMat, matrix4x4 projMat) {
    if (tree->numNodes > 0) {
        drawKDTreeNode(tree, 0, tree->bounds, viewMat, projMat);
    }
}

void drawMeshWireframe (object *object, matrix4x4 viewMat, matrix4x4 projMat) {
    ...
    for (unsigned int i = 0; i < mesh.numTriangles; ++i) {
        ...
        draw3DLine(p0.x, p0.y, p0.z, p1.x, p1.y, p1.z, viewMat, projMat);
        draw3DLine(p1.x, p1.y, p1.z, p2.x, p2.y, p2.z, viewMat, projMat);
        draw3DLine(p2.x, p2.y, p2.z, p0.x, p0.y, p0.z, viewMat, projMat);
    }
}


void renderScene (LARGE_INTEGER lastCounter, long long perfCountFrequency) {
    ...
    while (elapsedMS < targetMSPerFrame && currentRow < imageHeight) {
        vector3 color = {};
...
//                color += sampleReciprocal * calcColor(newRayPos, rayDir, 1);
        ...
        QueryPerformanceCounter(&workCounter);
        counterElapsed = workCounter.QuadPart - lastCounter.QuadPart;
        elapsedMS = (1000.0f * (float)counterElapsed) / (float)perfCountFrequency;
    }
...
int WINAPI WinMain (HINSTANCE instance, HINSTANCE prevInstance, LPSTR commandLine, int showCode) {
    ...
    // Show a console window, reopen standard streams
    AllocConsole();
    ...
    windowClass.lpszClassName = "EBRTClass";
    ...
    if (RegisterClassA(&windowClass)) {
        ...
        if (window) {
            ...
            while (programRunning) {
                ...
                // TODO(ebuchholz): Vsync/sleep until next frame after done rendering?
                if (!renderingFinished) {
                    renderScene(lastCounter, perfCountFrequency);
                }
